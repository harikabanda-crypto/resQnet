...
import { Home, ListChecks, Map, User, LogOut, Menu, X } from 'lucide-react'

const NAV = [
  { to: '/volunteer/dashboard', icon: Home, label: 'Home' },
  { to: '/volunteer/tasks', icon: ListChecks, label: 'Tasks' },
  { to: '/volunteer/map', icon: Map, label: 'Map' },
]
    ...
    <div className="min-h-screen bg-slate-50 flex">
      <aside className={`fixed inset-y-0 left-0 z-40 w-56 bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 shadow-sm ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div className="p-4 border-b border-slate-100 flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-green-500 to-green-700 flex items-center justify-center">🙋</div>
          <div>
            <div className="text-slate-800 font-bold text-sm">ResQNet</div>
            <div className="text-slate-400 text-[10px]">Volunteer Portal</div>
          </div>
        </div>
        <nav className="flex-1 p-3 space-y-0.5">
          {NAV.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to} onClick={() => setOpen(false)}
              className={({ isActive }) => `flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${isActive ? 'bg-green-50 text-green-600 font-semibold' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800'}`}>
              <Icon size={16}/>{label}
            </NavLink>
          ))}
        </nav>
      ...
      <div className="flex-1 lg:ml-56 flex flex-col">
        <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-slate-800 font-semibold text-sm">Volunteer – Arjun M.</span>
          </div>
          <div className="text-xs text-green-600 font-semibold border border-green-200 bg-green-50 px-2 py-0.5 rounded-full">AVAILABLE</div>
        </header>
